...
Logs:
			 "--Original RER Mod Tool 3ds Max script by maliwei777 \n"
			  ...
			  -- v0.8 aman - custom normal check box replace smooth overlap (idea from Irastris). 
					-- remove mesh MeshHeader.uiOffsetBoneMap[2] adjustment to avoid crash with sam_ceremonial and extra_mathias mods.
					-- not thoroghly tested, may break other characters that used to be moddable.
...
# Main
#===========================================================================
def do_export(self, context, props, filepath, bUseCustomNormal):
	SEEK_SET = 0
	SEEK_CUR = 1
	SEEK_END = 2
			...
			if bMeshSame == True:	# This mesh is marked as modified
				...
				for k in range(meshInfo[0]):  # check which group in mesh has changed
					...
					if k in MeshChange[i]:	#  this group need to be update						   
						...
						if len(OutMesh.vertices) > 0 and OutMesh.vertices[0].normal == None:
						...
						if bUseCustomNormal:
							...
							# aman: Blender calculates vertex normal using other vertices in same face which creates hard edges between disjointed faces in mesh
							# This code make all overlapped edges smooth. Use Blender custom normal instead if you want full control of vertex normal																	   
...
class Export_OT_Mod(bpy.types.Operator, ExportHelper):
	...
	def execute(self, context):
		...
		bUseCustomNormal = context.scene.CustomNormal
		exported = do_export(self, context, props, filepath, bUseCustomNormal)
...
class UIPanel(bpy.types.Panel):
	...
	name = "Property", description = "Property Name", default = 'lod')
	"""
	bpy.types.Scene.CustomNormal   = BoolProperty(default=False)
	...
	def draw(self, context):
		...
		layout.prop(scn, 'CustomNormal' , text ="Use Custom Normals")
